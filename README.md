...
- 사람 사용자 권한과 애플리케이션 실행 권한을 분리하는 구조를 전제로 설계
- 배포 파이프라인은 `GitHub Actions -> AWS Credentials -> ECR / EC2` 흐름으로만 필요한 권한을 사용
- 애플리케이션 런타임 권한은 서비스 단위로 분리해 `auth-service`, `ledger-service`, `ocr-ai-service`가 동일한 광역 권한을 공유하지 않도록 구성
- 문서 기준 권장 역할 분리는 `task-role-auth`, `task-role-ledger`, `task-role-ocr`이며, 각 서비스는 자신이 접근하는 AWS 리소스에만 권한을 부여
- 예시로 `ledger-service`는 영수증 업로드를 위한 `S3` 접근, `ocr-ai-service`는 `Textract` 호출과 OCR 처리용 저장소 접근, 배포 주체는 `ECR` 이미지 푸시와 `EC2` 배포 권한만 갖는 방식이 적합
- 공통 원칙은 `리소스 범위 제한`, `서비스별 역할 분리`, `장기 관리자 권한 미사용`, `시크릿은 코드가 아닌 AWS 비밀 저장소에서 주입`이다

### 6-5. 접근 제어 구조

- 1차 접근 제어는 `auth-service`가 발급한 JWT로 수행
- `ledger-service`는 요청마다 토큰을 검증하고, 토큰 안의 `sub(user_id)`와 `plan` 클레임을 읽어 사용자 범위와 기능 권한을 결정
- 데이터 접근은 역할 기반이라기보다 `사용자 스코프 + 플랜 기반 기능 제어`에 가깝다
- 예를 들어 거래/대시보드 조회는 항상 `Transaction.user_id == current_user["user_id"]` 조건으로 자기 데이터만 조회하며, PRO 전용 기능은 `plan == "PRO"` 조건을 추가로 검사
- 이 구조 덕분에 인증, 데이터 접근 범위 제한, 유료 기능 제어가 각각 분리되어 있고, 서비스 내부에서는 공통 `get_current_user` 의존성으로 동일한 검증 방식을 재사용한다
...
- FREE 사용자는 월 `50회`까지 OCR 업로드를 사용할 수 있고, PRO 사용자는 무제한으로 처리
- 사용량은 `ocr_usage_monthly` 테이블에 `user_id + yyyymm(YYYYMM)` 복합 키로 월별 저장
- 업로드 요청이 들어오면 먼저 `check_ocr_limit()`가 현재 월 사용량을 조회하고, FREE 사용자가 `50회 이상`이면 즉시 `403`을 반환
- 제한 검사를 통과한 뒤에만 `S3 업로드 -> ocr-ai-service 호출 -> 결과 저장` 순서로 진행
- 사용량 증가는 OCR과 저장이 모두 성공한 뒤 `increment_ocr_usage()`에서 `used_count`를 `+1` 하는 방식이라, 실패 요청은 기본적으로 차감하지 않음
- 월이 바뀌면 `yyyymm` 값이 달라지므로 새 레코드가 생성되고 사용량이 자연스럽게 새 달 기준으로 집계
- 설정 화면과 구독 화면은 `/ledger/ocr/usage` 응답의 `used_count`를 사용해 FREE 사용자는 `현재 사용량 / 50`, PRO 사용자는 `Unlimited`로 표시
...
- 인증 정보는 JWT로 전달되고, 각 백엔드 서비스는 토큰 서명을 검증한 뒤 `user_id`, `plan` 값을 추출해 사용
- 사용자 데이터 접근은 항상 `user_id` 기준으로 제한해 다른 사용자의 거래/영수증/통계 데이터에 접근하지 못하도록 설계
- 기능 권한은 `FREE`와 `PRO` 플랜으로 분리되며, 현재 구현상 대표적인 PRO 전용 기능은 AI 인사이트 조회이다
- 결제 상태는 `auth-service`의 Stripe 동기화 및 webhook 처리 결과로 `users.plan`, `subscription_status`, `next_billing_at`에 반영되고, 이후 다른 서비스가 JWT의 플랜 정보를 기반으로 기능 허용 여부를 판단

### 10-2. 사용량 제한과 결제 플랜 연동

- FREE 사용자는 OCR 월 50회 제한이 적용되고, PRO 사용자는 제한 없이 영수증 OCR을 요청할 수 있다
- 사용량 제한은 API Gateway나 WAF의 요청 수 제한과 별개로, 애플리케이션 레벨에서 `월별 기능 사용량`을 제어하는 비즈니스 룰이다
- 따라서 짧은 시간에 많은 요청을 막는 Rate Limit과 달리, 이 기능은 `이번 달에 실제로 몇 번 성공적으로 OCR을 사용했는가`를 기준으로 동작한다
- Stripe 결제 완료 후 구독 상태가 `PRO`로 동기화되면 이후 OCR 제한 검사에서 바로 무제한 처리 경로를 타게 된다
- 반대로 구독 취소 또는 해지 이벤트가 반영되어 `FREE`로 내려오면 다음 OCR 요청부터 다시 월 50회 제한이 적용된다
