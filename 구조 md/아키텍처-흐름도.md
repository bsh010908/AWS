...
flowchart LR
    U["User"]
    FE["Frontend (localhost:5500)"]

    subgraph DockerNetwork["Docker Network"]
      AUTH["auth-service (8001 -> 8000)"]
      LEDGER["ledger-service (8002 -> 8000)"]
      OCR["ocr-ai-service (8003 -> 8000)"]
      DB[("MySQL (3307 -> 3306)")]
    end
    ...
    FE -->|회원가입/로그인/내정보| AUTH
    FE -->|거래/예산/대시보드/카테고리| LEDGER
    FE -->|직접 OCR 호출(선택)| OCR

    LEDGER -->|영수증 분류 요청 (POST /ocr/classify)| OCR
    AUTH --> DB
    LEDGER --> DB

    STRIPE -->|POST /billing/webhook| AUTH
```
